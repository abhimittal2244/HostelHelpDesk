...
namespace HostelHelpDesk.Persistence.Data
{
    ...
    //        modelBuilder.Entity<WorkerWorkerType>()
    //.HasKey(wwt => new { wwt.WorkerId, wwt.WorkerTypeId });

    //        modelBuilder.Entity<WorkerWorkerType>()
    //            .HasOne(wwt => wwt.Worker)
    //            .WithMany(w => w.WorkerWorkerTypes)
    //            .HasForeignKey(wwt => wwt.WorkerId);

    //        modelBuilder.Entity<WorkerWorkerType>()
    //            .HasOne(wwt => wwt.WorkerType)
    //            .WithMany(wt => wt.WorkerWorkerTypes)
    //            .HasForeignKey(wwt => wwt.WorkerTypeId);

            modelBuilder.Entity<WorkerType>()
                .HasOne(ct => ct.ComplaintType)
                .WithMany()
                .HasForeignKey(ct => ct.ComplaintTypeId)
                .OnDelete(DeleteBehavior.Restrict);
            ...
            //    .HasMany(ct => ct.EligibleWorkerTypes)
            //    .WithMany(wt => wt.ComplaintTypes)
            //    .UsingEntity(j => j.ToTable("ComplaintTypeWorkerType"));
            ...
            modelBuilder.Entity<Room>()
                .HasIndex(x => new { x.HostelId, x.RoomNo })
                .IsUnique();
        }
    }
}
